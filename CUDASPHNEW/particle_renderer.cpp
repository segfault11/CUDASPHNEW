    ...
    unsigned int width, unsigned int height): _sim(&sim)
{
	...
	cgtkGLAttachShader(_program, "particle_renderer_v.glsl", 
        GL_VERTEX_SHADER);
	cgtkGLAttachShader(_program, "particle_renderer_f.glsl", 
        GL_FRAGMENT_SHADER);
	cgtkGLAttachShader(_program, "particle_renderer_g.glsl", 
        GL_GEOMETRY_SHADER);
